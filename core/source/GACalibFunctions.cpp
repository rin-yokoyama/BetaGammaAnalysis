...
GACalibFunctions::GACalibFunctions(string fname, Int_t fnum)
{
    f_num = fnum;
    f_name = fname;
    range[0] = 0;
...
GACalibFunctions::~GACalibFunctions(void) {}

void GACalibFunctions::SetRange(Double_t xlow, Double_t xup)
{
    range[0] = xlow;
    range[1] = xup;
}

TF1 *GACalibFunctions::CalibFunc(CalFuncType func_type)
{
    TF1 *fTF1;
    ostringstream oss;
    oss.str("");
    ...
    if (func_type == kHyperbolic)
        fTF1 = new TF1(oss.str().c_str(), "[0]*(x-[3])-sqrt([1]*(x-[3])^2 + [2])+[4]", range[0], range[1]);
    return (TF1 *)fTF1->Clone();
}

Double_t GACalibFunctions::CalcResidualError(Double_t ch, Double_t che, Double_t RefEe, Double_t *par, Double_t *parE, CalFuncType func_type)
{
    Double_t fitE;
    if (func_type == kLine)
    {
...
void GACalibFunctions::SetInitialParam(TF1 *fTF1, Double_t gain, Double_t offset, CalFuncType func_type)
{
    if (func_type == kLine)
    {
    ...
    {
        fTF1->SetParameters(fDivisionPoint, offset, gain, offset, gain);
    }
    else if (func_type == kHyperbolic)
    {
